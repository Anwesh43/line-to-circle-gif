...
class LineToCircleGif {
    ...
    create(fileName) {
        ...
        renderer.render(this.context, this.lineToCircle, (context) => {
...
class LineToCircle {
    ...
    draw(context) {
        ...
        const r = size/9
        context.save()
        context.translate(size/8, size/2)
        ...
        this.drawArc(context, 180, 360 * (this.state.scales[1]), r)
        context.restore()
        context.restore()
    }
    update(stopcb) {
...
class Renderer {
    ...
    render(context, lineToCircle, updatecb, stopcb) {
        while(this.running) {
